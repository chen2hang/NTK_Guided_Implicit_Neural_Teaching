...
class SSIM(torch.nn.Module):
    ...
    def forward(self, img1, img2):
        ...
        return _ssim(img1, img2, window, self.window_size, self.channel, self.size_average, stride=self.stride)
...
def compute_ssim_loss(img1, img2):
    ...
    # return t_ssim(img1, img2, size_average=True)

    img1 = img1.unsqueeze(0).to(torch.float32) / 255.0 # 1,c,h,w
    img2 = img2.unsqueeze(0).to(torch.float32) / 255.0
