...
class AudioFileDataset(torchaudio.datasets.LIBRISPEECH):
    def __init__(
        ...
        # Use configurable root path, default to 'data' for backward compatibility
        root = getattr(dataset_configs, 'file_path', 'data')
        super().__init__(root=root, url='test-clean', download=False)
        ...
        # For sampler compatibility: treat 1D audio as 1xT "image"
        self.H, self.W = 1, self.T
